...
(function () {
  ...
  function openAdmin() {
    ...
    if (lockEnabled() && !unlocked) {
      ...
      setTimeout(() => $('#gatePass').focus(), 60); return;
    }
    showPanel();
  }
  ...
  function renderBackupTab() {
    ...
    $('#bkLock').addEventListener('change', (e) => {
      if (e.target.checked) {
        ...
        window.Athar.toast('سيُطلب الرمز عند الفتح من الآن');
      } else { localStorage.removeItem(LOCK_KEY); window.Athar.toast('الدخول مباشر بدون رمز'); }
    });
